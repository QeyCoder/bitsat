    ...
    static class Solution {
        ...
        public List<Boolean> checkIfPrerequisite(int n, int[][] prerequisites, int[][] queries) {
            ...
            for (int[] query : queries) {
                int src = query[0];
                int dest = query[1];
                if (store.contains(new Node(src, dest))) {
                    ans.add(true);
                } else {
                    Boolean a = check(src, dest);
        ...
        private Boolean check(int src, int dest) {

            Vertex<Integer> edges = graph.getVertex(src);
            if (src == dest) {
                return true;
            }
            if (edges!=null && edges.getAdjacentVertex().size() > 0) {
                for (Vertex<Integer> vertex : edges.getAdjacentVertex()) {
                    if(!visited[vertex.getVal()]){
                        Boolean ans = check(vertex.getVal(), dest);
                        if (ans) {
                            return ans;
                        }
                    }
